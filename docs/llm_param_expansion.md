...
|------|------|------------|-----------|-------------|-------------------|--------|
| Qwen3-7B | 28 | 3584 | 28 | 4 | 18944 | 7.6B |
| Qwen3-14B | 40 | 5120 | 40 | 8 | 13824 | 14.8B |
| Qwen3-32B | 64 | 5120 | 64 | 8 | 25600 | 32.8B |
| Qwen3-72B | 48 | 8192 | 48 | 12 | 29568 | 72.4B |

**对扩增的启示**：
- Qwen3 系列采用 **GQA + SwiGLU FFN**，与 LLaMA-Pro/MSG 的假设一致，可直接套用恒等块/掩码生长
- 7B→14B 的扩增目标与官方 14B 模型的层数/宽度配置不同，说明“翻倍参数”不等于复制官方尺寸
- 32B 与 72B 的 hidden_size 差异（5120 vs 8192）提示：宽度扩展需与 FFN 尺寸协同设计，避免比例失衡
...
```
原始模型:  Qwen3-7B, 28 层, hidden_size=3584, 约 7.6B 参数
扩展策略（论文风格 / 保守）:
  每 4 个原始块后插入 1 个恒等块 → 28 + 7 = 35 层, ~9.5B 参数

扩展策略（激进 / 接近 2x）:
  每 1 个原始块后插入 1 个恒等块 → 28 + 28 = 56 层, ~14B 参数
...
```
原始模型: Qwen3-7B (28层, d=3584)
生长策略: 组合扩展（需配合 MSG 掩码逐步解锁新维度）
  - 深度: 28 → 36 层 (Transformer block 参数增 ~29%)
  - 宽度: 3584 → 4096 (Attention/Embedding 相关参数增 ~31%)
  - FFN: 18944 → 22528 (MLP 参数增 ~19%)
  总参数增: ~1.29 × 1.31 × 1.19 ≈ 2x（实际略低于 2x，因 Embedding/LM Head 不变）
...
```
原始模型: Qwen3-7B (约 7.6B 参数，FFN 约占 5.5-6B)
每层 FFN 复制 8 份 → 8 Expert, top-1 路由
  总参数: ~7.6B - 5.7B + 8 × 5.7B ≈ 47B
...
```
Step 1: LLaMA-Pro 扩展 Qwen3-7B → ~14B 骨架
Step 2: 用 Qwen3-72B 为 teacher 蒸馏
