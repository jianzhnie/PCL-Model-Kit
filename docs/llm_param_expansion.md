...
### 方案 M4：Attention 层扩展（d\_model / num\_heads）
...
- 增加 `num_attention_heads` / `num_kv_heads`：新增头的 o\_proj 初始化为零
...
<img src="svg/16_combined_expansion.svg" alt="M1+M2 Width" width="780"/>
...
| 模型        | 层数 | hidden\_size | num\_heads | num\_kv\_heads | intermediate\_size | 参数量     |
| --------- | -- | ------------ | ---------- | -------------- | ------------------ | ------- |
...
- 14B 与 32B 的 hidden\_size 相同（5120），主要差异在层数（40 vs 64）和 intermediate\_size（17408 vs 25600），提示：深度扩展是大模型扩参的高效路径
