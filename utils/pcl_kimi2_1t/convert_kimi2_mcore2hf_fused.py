...
"""
Kimi2 MCore to HuggingFace 权重转换脚本 (Fused QKV variant)

与原版不同之处: attention 权重以 fused_qkv (single linear) 格式保存,
与 DeepseekV3FusedQKVAttention 兼容, 无需推理时再调用 convert_to_fused_qkv_attn.
...
class MgCkptConvert(object):
    ...
    def set_model_attn(self, hf_dict, mg_models, hf_layer_idx,
                       local_layer_idx):
        """
        GQA Attention 转换 — Fused QKV variant
        ...
        输出格式 (兼容 DeepseekV3FusedQKVAttention):
        - fused_qkv: [Q_all; K_all; V_all, hidden_size]  (single fused linear)
        ...
        # Fused QKV: [Q_all; K_all; V_all] — same layout as DeepseekV3FusedQKVAttention
        fused_qkv = torch.cat([q_proj, k_proj, v_proj], dim=0)
        ...
        hf_dict[
            f"model.layers.{hf_layer_idx}.self_attn.fused_qkv.weight"] = fused_qkv.clone(
            )
        hf_dict[
